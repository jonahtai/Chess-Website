...
async function search() {
    ...
    debounceTimeout = setTimeout(async() => {
        try {
            const response = await fetch(`http://143.47.99.139:8000/api/search?query=${encodeURIComponent(query)}`);
            if (response.ok) {
...
async function getLeaderboard() {
    ...
    try {
        const response = await fetch(`http://143.47.99.139:8000/api/leaderboard?schools=${encodeURIComponent(schools)}&minRating=${encodeURIComponent(minimumRating)}&maxRating=${encodeURIComponent(maximumRating)}`);
        if (response.ok) {
